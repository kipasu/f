...
#wget -O /home/vps/public_html/index.html "${REPO}ssh/index.html1"
...
cat> /etc/cron.d/keep_cpu << END
SHELL=/bin/sh
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
*/5 * * * * root /etc/autocpu.sh
END
wget -O /usr/bin/autocpu "${REPO}ssh/autocpu.sh" && chmod +x /usr/bin/autocpu
